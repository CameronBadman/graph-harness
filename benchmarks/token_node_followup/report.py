...
COMPARISONS = [('native', arm) for arm in ARMS[1:]] + [('compact', 'slim'), ('node', 'combined'), ('compact', 'node'), ('slim', 'combined')]
...
def _command(frozen, row, folder):
    ...
    spec = {'writable': True} if row['task'] == 'calibration' else frozen['tasks'][row['task']]
    ...
    if row['arm'] != 'native':
        args = trials.bridge_args(source_workspace, row['arm'])
...
def _verify_profile(row, folder):
    capabilities = _load(folder / 'capabilities.json')
    if capabilities.get('coordinated_writes') is not True or digest(stable(capabilities)) != row.get('capabilities_sha256'):
        raise ValueError('write-enabled daemon capability evidence differs')
    contract = _load(folder / 'mcp-contract.json')
    definitions = _load(folder / 'tool-definitions.json')
    if (contract.get('tools') != definitions or digest(stable(contract)) != row.get('mcp_contract_sha256')
            or {tool['name'] for tool in definitions} != trials.expected_tools_for(row['arm'])
            or row.get('tools') != [tool['name'] for tool in definitions]
            or row.get('tool_schema_sha256') != digest(stable(definitions))
            or row.get('tool_schema_json_bytes') != len(stable(definitions))):
        raise ValueError('actual tool profile differs from registered treatment')
    command = _load(folder / 'command.json')
    daemon = _load(folder / 'daemon-command.json')
    workspace = command[command.index('--cd') + 1]
    launcher = json.loads(next(value.split('=', 1)[1] for value in command if value.startswith('mcp_servers.graphharness.command=')))
    if daemon != [launcher, 'daemon', workspace, '--allow-edits']:
        raise ValueError('daemon invocation differs from write-enabled treatment')
    return capabilities
...
def _verify_calibration(base, frozen, reviews):
    ...
    for row in calibrated:
        ...
        with tempfile.TemporaryDirectory(prefix='gh-calibration-recheck-') as temporary:
            original = Path(temporary)
            (original / 'Gauge.java').write_text('public class Gauge { public int reading() { return 37; } }\n')
            specification = trials.calibration_spec(original, row['arm'], frozen['integration_guidance'])
        prompt = _bound_bytes(folder, 'prompt.txt', row.get('prompt_sha256'))
        schema = _load(folder / 'schema.json')
        if (prompt.decode() != specification['prompt'] or schema != specification['response_schema']
                or row.get('schema_sha256') != digest(stable(schema))
                or row.get('fixture_sha256') != specification['manifest_sha256']
                or row.get('workspace_before_sha256') != specification['manifest_sha256']
                or row.get('workspace_after_sha256') != trials.tasks.manifest(folder / 'workspace')):
            raise ValueError('calibration prompt, schema or workspace evidence differs')
        ...
        diagnostic = trials.diagnostics(events)
        ...
        expected_correctness = trials.calibration_result(folder / 'workspace', answer.decode(), diagnostic, row['arm'])
        if (expected_correctness != {'passed': True} or row.get('correctness') != expected_correctness
                or row.get('instrumentation_valid') is not True or row.get('exit_code') != 0
                or row.get('timed_out') is not False or row.get('infrastructure_failure')):
            raise ValueError('calibration no longer passes correctness, route and instrumentation')
        _command(frozen, row, folder)
        if row['arm'] != 'native':
            _verify_profile(row, folder)
...
def collect(base):
    ...
    if frozen.get('schedule_seed') != 2026091805 or frozen.get('schedule') != _schedule():
    ...
    if frozen['artifact_hashes'].get('benchmarks/token_node_followup/tasks.py') != evaluator_hash:
    ...
    preflight_rows = preflight.verify(base, frozen)
    frozen["preflight_summary"] = [{key: row[key] for key in ("task", "passed", "committed", "plan_created", "file", "node_id", "original_file_hash", "file_hash", "admission", "correctness", "workspace_before_sha256", "workspace_after_sha256", "frozen_sha256", "evidence_sha256")} for row in preflight_rows]
    frozen["preflight_sha256"] = digest((base / "preflight.json").read_bytes())
    ...
    for raw in raw_rows:
        ...
        if row.get('preflight_sha256') != digest((base / 'preflight.json').read_bytes()):
            raise ValueError('scored run is not bound to target preflight')
        ...
        if raw_events is not None:
            ...
            if events is not None:
                ...
                diagnostics = trials.diagnostics(events)
        ...
        if definitions is not None:
            ...
            if row['arm'] != 'native' and set(row['tools']) != trials.expected_tools_for(row['arm']):
                raise ValueError('navigation profile does not expose registered tools')
            if row['arm'] != 'native':
                capabilities = _verify_profile(row, folder)
...
def summarize(rows, frozen=None):
    ...
    for arm in ARMS:
        ...
        arms[arm] = {
            ...
            'node_edit_attempts': sum(row.get('diagnostics', {}).get('node_write_calls', 0) > 0 for row in attempts),
            'node_edit_committed_runs': sum(row.get('diagnostics', {}).get('node_write_commits', 0) > 0 for row in attempts),
            'source_v1_runs': sum(row.get('diagnostics', {}).get('source_v1_bundles', 0) > 0 for row in attempts),
    ...
    for arm in ARMS[1:]:
        ...
        if arm in trials.NODE_ARMS:
            checks['node_edit_in_at_least_half_attempts'] = summary['attempts'] > 0 and summary['node_edit_committed_runs'] * 2 >= summary['attempts']
        if arm in trials.SLIM_ARMS:
            checks['source_v1_in_at_least_half_attempts'] = summary['attempts'] > 0 and summary['source_v1_runs'] * 2 >= summary['attempts']
...
def _sanitized(row):
    fields = ('id', 'task', 'arm', 'repetition', 'usage', 'correctness', 'instrumentation_valid',
              ...
              'prompt_sha256', 'schema_sha256', 'frozen_sha256', 'preflight_sha256', 'evaluator_sha256', 'raw_events_sha256',
              'stderr_sha256', 'answer_sha256', 'tool_schema_sha256', 'tool_schema_json_bytes', 'tools',
              'mcp_contract_sha256', 'capabilities_sha256', 'admission', 'backend_sha256', 'diagnostics', 'access_audit', 'correctness_rechecked',
...
def render(frozen, rows, summary, output):
    ...
    metadata = {name: frozen[name] for name in ('schema_version', 'study', 'task_provenance', 'model', 'model_identity', 'codex_version',
                                               'repository_head', 'schedule_seed', 'schedule', 'artifact_hashes')}
    metadata['preflight_required'] = True
    metadata['preflight_sha256'] = frozen['preflight_sha256']
    trials.write_json(output / 'preflight.json', frozen['preflight_summary'])
    ...
    with (output / 'runs.csv').open('w') as stream:
        names = ['id', 'task', 'repetition', 'arm', 'correct', 'valid', *METRICS,
                 'elapsed_seconds', 'setup_seconds', 'read_retrieval_attempted', 'node_edit_committed', 'source_v1_used', 'policy_rejections', 'access_status']
        ...
        for row in rows:
            writer.writerow({**{key: row.get(key) for key in names if key in row},
                             **{metric: (row.get('usage') or {}).get(metric) for metric in METRICS},
                             ...
                             'node_edit_committed': row.get('diagnostics', {}).get('node_write_commits', 0) > 0,
                             'source_v1_used': row.get('diagnostics', {}).get('source_v1_bundles', 0) > 0,
                             'policy_rejections': row.get('diagnostics', {}).get('stderr_policy_rejections'),
                             'access_status': row.get('access_audit', {}).get('status')})
    lines = ['# Repaired node editing: guided development rerun', '',
             ...
             'Every correctness result was recomputed by the external evaluator. Target families require separate real node-commit preflight. Tasks are reused development data; explicit editing guidance and a common validation instruction changed before this freeze.', '',
    ...
    for arm, result in summary['arms'].items():
        ...
        lines += [f"| {arm} | {result['correct_attempts']} / {result['valid']} / {result['attempts']} | "
                  f"{result['retrieval_attempts']} / {result['node_edit_committed_runs']} / {result['source_v1_runs']} | {result['policy_rejections']} | "
    ...
    lines += ['', 'Known totals include invalid and incorrect attempts with available counters. '
              ...
              'requiring all five configurations to be valid and correct. Positive values mean fewer tokens; '
    ...
    lines += ['', '## Registered continuation threshold', '',
              ...
              'in at least half the treatment attempts; node/format arms also require their intended mechanism in at least half. Incomplete common measurement coverage cannot pass.', '']
    ...
    lines += ['', '## Limits and evidence', '',
              'Three reused generated tasks, two repetitions and one shared source corpus constitute a guided development rerun, '
              'not a production benchmark or statistical generalization. Provider cache state is uncontrolled. '
              'All graph arms receive identical explicit integration guidance, including its input overhead; native does not. '
              'Retrieval selection stays fixed. Format and guided edit availability vary factorially. Known task results informed repairs; this is not a fresh holdout. A common compilation/behavior-check requirement applies to every arm. Low adoption limits attribution. '

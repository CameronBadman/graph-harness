...
from benchmarks.token_node_followup import report
...
def dataset():
    with tempfile.TemporaryDirectory() as temporary:
        base = Path(temporary)
        identity = {'benchmarks/token_node_followup/tasks.py': report.digest(Path(report.trials.tasks.__file__).read_bytes()),
                    'product/artifact': 'registered product hash'}
        with patch.object(report.preflight, 'verify', return_value=[]), \
                patch.object(report.trials, 'identity', return_value=identity) as identity_mock, \
            ...
            frozen = {'schema_version': 1, 'model': 'configured-test-model', 'model_identity': 'not attested',
                      ...
                      'repository_head': 'unit-fixture', 'study': 'guided_node_development_rerun', 'task_provenance': 'reused development data', 'flags': ['--json'], 'schedule_seed': 2026091805,
            ...
            write_json(base / 'preflight.json', {'synthetic': 'separate preflight validation tests cover this gate'})
            ...
            for registered in frozen['schedule'] + [{'id': 'calibration-' + arm, 'arm': arm,
                ...
                row.update(usage=report.trials.support.parse_usage(events), correctness={'passed': True} if calibration else evaluate(
                    ...
                    evaluator_sha256=identity['benchmarks/token_node_followup/tasks.py'],
                    preflight_sha256=report.digest((base / 'preflight.json').read_bytes()),

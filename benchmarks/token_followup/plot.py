...
def render(directory):
    ...
    if len(comparisons) != 3 or not summary["common_cells"]:
        raise ValueError("three configurations and common valid/correct cells are required")
    labels = {"legacy": "Preserved old build", "repaired": "Repaired full tools", "navigation": "Navigation profile"}
    plt.rcParams.update({"font.family": "DejaVu Sans", "svg.hashsalt": "graphharness-token-followup"})
    ...
    for axis, metric, title in zip(axes, ("input_tokens", "output_tokens"), ("Input tokens", "Output tokens")):
        ...
        axis.barh(range(3), values, color=["#24845b" if value >= 0 else "#b44442" for value in values])
        axis.set_yticks(range(3), [labels[row["treatment"]] for row in comparisons])
        axis.invert_yaxis()
    ...
    fig.suptitle(f"Actual Codex usage · {count} matched correct task/repetition cells\n"
                 "Negative percentages mean MORE tokens; fresh generated pilot", fontsize=13)

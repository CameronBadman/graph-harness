...
from benchmarks.token_interface import run_trials as trials
...
def collect(base):
    ...
    if frozen.get('schedule_seed') != 2026091804 or frozen.get('schedule') != _schedule():
    ...
    if frozen['artifact_hashes'].get('benchmarks/token_interface/tasks.py') != evaluator_hash:
...
def render(frozen, rows, summary, output):
    ...
    lines = ['# Slim interface and node-edit factorial pilot', '',
    ...
    lines += ['', '## Limits and evidence', '',
              'Three generated tasks, two repetitions and one shared source corpus constitute a development confirmation, '
              ...
              'Retrieval selection stays fixed. Format and edit availability vary factorially. Low adoption limits attribution. '

...
**The token-saving goal remains unmet.** After fixing Java target matching and making editing guidance explicit, the latest [30-run development rerun](reviews/token-node-followup/FINDINGS.md) had **12/12 node-enabled runs use the editor**, and all 30 repairs passed. Node editing used **57.4% more input and 45.0% more output** than native Codex; format plus node editing used **52.7% more input and 34.1% more output**. Every graph configuration increased total input and output. These are three reused synthetic tasks, not an unseen-task or production estimate; formatting overhead also remains. The [prior interface study](reviews/token-interface/FINDINGS.md), [earlier 24-run follow-up](reviews/token-fix/FINDINGS.md) and [original pilot](reviews/token-ablation/FINDINGS.md) remain unchanged.
